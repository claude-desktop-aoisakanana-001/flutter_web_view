...
## 📢 アーキテクチャ決定（2025-11-30）

**決定**: **オプション A（WebView内表示）を採用**

利用規約調査の結果を受けて、本プロジェクトでは WebView + JavaScript Injection 方式を採用することを決定しました。
...
**実装方針**:
- `webview_flutter` パッケージを使用
- JavaScript Injection によるDOM取得
- 本ドキュメントで特定したセレクタを活用

詳細は [tech-selection.md](tech-selection.md)、[architecture.md](architecture.md)、[issues/issue-03.md](issues/issue-03.md) を参照。
