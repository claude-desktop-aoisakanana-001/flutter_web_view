...
### Issue #9: WebViewブラウザ実装とUI統合 ⚠️ **2025-12-27 更新**
...
**重要**: UXの根本的な変更 - テキストビューアから「サイトUI拡張」アプローチへ移行。

#### タスク
- [ ] `NovelReaderScreen` をWebView中心のレイアウトに変更
  - 全画面WebViewコンポーネント
  - 下部固定のコントロールパネル（速度設定 + 再生コントローラー）
  - URL入力フィールドの削除
- [ ] WebViewController の初期化と設定
  - 初期URL: `https://syosetu.com/` （暫定、後で変更可能）
  - JavaScript有効化、ナビゲーション設定
- [ ] `WebViewState` と `WebViewNotifier` の作成（Freezed + Riverpod）
  - URL変更の監視、ページ読み込み状態の管理
- [ ] TTSコントロールの表示制御準備
  - 常時表示、現時点では機能無効化（Issue #10で有効化）
- [ ] 既存コードの整理（削除ではなく使用停止）
  - `NovelContentView` の非表示化
  - URL入力関連UIのコメントアウト
- [ ] テスト作成
  - WebView初期化、初期URL確認、レイアウト構成のテスト

#### 成功基準
- [ ] アプリ起動時に小説家になろうのトップページが表示される
- [ ] WebView内でリンクをクリックしてページ遷移できる
- [ ] 下部にTTSコントロールが常時表示されている
- [ ] `flutter analyze` でエラーなし、テストが成功

---

### Issue #10: 小説ページ検出とTTS統合 ⚠️ **2025-12-27 更新**
...
#### タスク
- [ ] 小説ページURLパターンの定義と `NarouUrlDetector` の作成
...
- [ ] URL監視とページ検出機能
  ...
  - 小説ページ検出時のイベント処理
- [ ] 小説本文の自動抽出
  - `NarouParserService` の再利用（既存実装を活用）
  - WebController経由で本文を解析
- [ ] TTSコントロールの有効化制御
  - 小説ページ: ボタン有効化
  - 非小説ページ: ボタン無効化（グレーアウト）
- [ ] TTS再生機能との統合
  - 再生ボタン押下時に抽出済み段落を読み上げ
  - 段落単位での読み上げ制御、再生位置の管理
- [ ] ページ遷移時の処理
  - 再生中の自動停止、本文データのクリア
- [ ] テスト作成
  - URL検出、ページ検出、TTS統合のテスト

#### 成功基準
- [ ] 小説ページに遷移すると自動的に本文が抽出される
- [ ] 小説ページでTTSコントロールが有効、非小説ページで無効になる
- [ ] 再生ボタンで小説を読み上げできる
- [ ] ページ遷移時に適切に状態がクリアされる
- [ ] すべてのテストが成功、`flutter analyze` でエラーなし

---

## 5. Phase 4: 総合テストと調整

### Issue #11: WebViewブラウザ統合テストとUX改善 ⚠️ **2025-12-27 更新**
...
**重要**: 元々Issue #9, #10で予定していたハイライト・自動スクロール機能を統合。

#### タスク
- [ ] **読み上げ位置のハイライト表示機能**
  - JavaScript Injection で DOM を直接操作
  - 現在読み上げ中の段落に CSS class を追加
  - スタイル（背景色）をインジェクト
  - TtsService に進捗コールバックを追加
- [ ] **読み上げ位置の自動スクロール機能**
  - JavaScript Injection で `element.scrollIntoView()` を実行
  - スムーズスクロールオプションを有効化
  - 段落読み上げ開始時にスクロール
- [ ] 統合テストシナリオの実施
  1. アプリ起動
  ...
  14. 非小説ページへ遷移時の動作確認
- [ ] 実機でのテスト（iOS）
  - iPhone実機での動作確認、WebViewのレンダリング確認
  - パフォーマンスチェック、メモリ使用量チェック、バッテリー消費チェック
- [ ] エッジケースのテスト
  - ネットワークエラー時の動作
  - 長文小説、ルビが多い小説、特殊記法がある小説での動作
  - 短編小説での動作、ページ遷移パターンのテスト
- [ ] UI/UX調整
  - レイアウト、色・フォント、アニメーションの調整
  - エラーメッセージの改善、アクセシビリティ確認
- [ ] パフォーマンス最適化
  - 不要な rebuild の削減
  - WebView のメモリ使用量最適化
...
- [ ] コードレビューとリファクタリング
  - コードの可読性チェック、コメント追加
  - 未使用コード（`NovelContentView`など）の削除
- [ ] ドキュメント更新
  - README.md の更新（WebViewベースのブラウザであることを明記）
  - `documents/known-issues.md` の作成
  - `documents/mvp-completion-report.md` の作成
...
## 6. Issue間の依存関係図 ⚠️ **2025-12-27 更新**
...
[Phase 3: WebViewブラウザ統合] ⚠️ 変更
Issue #6 + Issue #7 + Issue #8
    └─→ Issue #9 (WebViewブラウザ実装) ← UXの根本的変更
...
【変更点】
- Issue #9: ハイライト表示 → WebViewブラウザ実装
- Issue #10: 自動スクロール → ページ検出とTTS統合
- Issue #11: ハイライト・自動スクロール機能を統合
```

---

## 7. 作業スケジュール（参考） ⚠️ **2025-12-27 更新**

| Phase | Issues | 合計見積もり | 備考 |
|-------|--------|------------|------|
| Phase 1 | #1〜#4 | 6.5時間 | 基盤構築、並行作業可能な部分あり |
| Phase 2 | #5〜#7 | 6時間 | TTS機能、#6と#7は#5の後に並行可能 |
| Phase 3 | #8〜#10 | 9時間 | WebViewブラウザ統合（見積もり増加） |
| Phase 4 | #11 | 4時間 | 総合テスト + ハイライト + 自動スクロール |
| **合計** | **11 Issues** | **25.5時間** | 約3〜4日間の作業量 |

**注**: 見積もりは参考値。実際の作業時間は個人差あり。
**変更点**: Phase 3とPhase 4の見積もりが増加（WebViewブラウザ実装とJavaScript Injection追加のため）
...
| 2025-12-27 | 2.0.0 | **根本的な変更**: Issue #9〜#11を全面的に再構成。UXの根本的な変更 - テキストビューアから「WebViewベースのサイトUI拡張」アプローチへ移行。Issue-08完了後のフィードバックを受けて、ブラウザとしての自然な閲覧体験を提供する設計に変更 |
